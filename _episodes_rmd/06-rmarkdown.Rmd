...
keypoints: Rmd is a popular ... for creating reproducible reports, articles, and theses.
source: Rmd
questions:
- What is R Markdown?
...
R Markdown is a flexible type of document that allows you to combine code and
...
data, without making any changes in the actual document (unless your interpretations
change drastically).
...
To create a basic output, a YAML (Yet Another Markup Language) header is needed.
...
we will begin writing below the YAML header (after the second `---`).
...
so we will need the data we worked with previously.

That means we need to make sure **tidyverse** is loaded:

```{r load-package, message=FALSE, purl=FALSE}
library(tidyverse)
```

And, if not still in the workspace, load the data we saved in the previous lesson:
...
Next, we will create a table with the first 10 entries in the data frame.

For that, we will need to create a 'code chunk'. The syntax of a code chunk is:

![](../fig/rmd-code-chunk.png)
...
This `r print('R-code')` is surrounded by meaningful text.
